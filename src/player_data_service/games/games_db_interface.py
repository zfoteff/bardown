...
from bardown_lib.errors.games_errors import GameAlreadyExists, GameDoesNotExist
    ...
    def get_game_results(self, game_id) -> List[GameResultDTO]:
        query = f"""
            ...
            from
                ...
                inner join game_statistics gs on gs.playerid = p.playerid
                and gs.gameid = g.gameid
            where
                g.gameid = "{game_id}"
        """
        success, result = self.__client.execute_query(query, return_results=True)
